...
#gc3 gabc with {init1}{init2}{commentary} and English spacing
  if ($INLINE =~ m/<g3e>(.*?)<\/g3e>/) {
  $TEXT = $INLINE;
  $TEXT =~ s/<g3e>//;
  $TEXT =~ s/<\/g3e>.*//;
...
#  print "$SCCOUNT\n";
  ...
  system ("/home/ahinkley/gprocess_eps_quiet $GABCNAME$SCCOUNT 4.5");
  $GABCOUT =~ s/.gabc$/.tex/;
  print OUTPUTTEMP "\\init1\{$INIT1\}\n";
  print OUTPUTTEMP "\\init2\{$INIT2\}\n";
...
\newcommand{\ginit}[2]{\gresetfirstlineaboveinitial{\small \textsc{\textbf{#1}}}{\small \textsc{\textbf{#1}}} \setsecondannotation{\small \textsc{\textbf{#2}}}}
